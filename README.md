# iOS App Portfolio

## 自己紹介

**𠮷留　徹（よしどめ　とおる）**

- 📧 Email: kamui00002@yahoo.co.jp
- 📱 TEL: 080-4274-1778

AIとノーコードツールを活用し、個人でiOSアプリの企画から開発、リリースまでを行っています。
React Native、Firebase、Xcodeなどを使用し、Udemy、YouTube、オープンチャットコミュニティなどで
情報を収集しながら、独学でアプリ開発に取り組んでいます。
Claude・Cursorなどの最新AI開発ツールも積極的に取り入れ、
試行錯誤を重ねながら、日々スキルアップを目指しています。

---

## 開発実績

### 公開済みアプリ（2個）

#### 1. AI-compass
[![App Store](https://img.shields.io/badge/App%20Store-0D96F6?style=for-the-badge&logo=app-store&logoColor=white)](https://apps.apple.com/jp/app/id6753145777)

<img src="./images:ai-compass:screenshot1.png" width="250" alt="AI-compass スクリーンショット">

今必要としているAIの機能や調べたAIにどんな機能があるかを検索できるアプリです。
「やりたいことからAIを探す」機能や、選択肢で探せる「AI診断」機能も搭載しています。

- **開発期間**: 2024年8月〜2024年9月
- **使用技術**: 
  - React 19.1.1
  - TypeScript 5.8.2
  - React Router DOM 7.9.2（HashRouter）
- **ダウンロード数**: 2

[詳細を見る →](./apps/ai-compass.md)

---

#### 2. ことばのたまご
[![App Store](https://img.shields.io/badge/App%20Store-0D96F6?style=for-the-badge&logo=app-store&logoColor=white)](https://apps.apple.com/jp/app/id6754462721)

自分の性格（MBTI）タイプに合わせたキャラクターを育てながら英単語を学べるアプリです。
卵からキャラクターが孵化し、学習するほど成長します。MBTI診断×語彙学習の新しいアプリです。

- **開発期間**: 2024年9月〜2024年11月
- **使用技術**:
  - Expo (~54.0.18)
  - React (19.1.0)
  - React Native (0.81.5)
  - TypeScript
- **ダウンロード数**: 5

[詳細を見る →](./apps/kotoba-no-tamago.md)

---

### 開発中アプリ（2個）

#### 3. ParkPedia
🔄 **現在の状況**: App Store審査中

日本中の人がレビューや写真を投稿し、みんなでおすすめの公園情報を集めて完成させるコミュニティ型アプリです。

- **使用技術**: React Native、Firebase

[詳細を見る →](./apps/parkpedia.md)

---

#### 4. EscapeNine（9マスゲーム）
🔄 **現在の状況**: 要件定義作成中

9マス鬼ごっこを題材に、リズムゲームのような感覚でクリアしていくアプリです。

[詳細を見る →](./apps/escape-nine.md)

---

## 開発スタイル

### AI活用による効率的な開発

- **Claude・Cursor等のAIツール**を積極的に活用
- コード生成時には必ず説明コメントを付けてもらい、自分でも学習しながら開発
- AIの力を借りつつ、仕組みを理解することを重視

### 独学での学習姿勢

- **Udemy**での体系的な学習
- **YouTube**での技術情報収集
- **オープンチャットコミュニティ**での情報交換
- 分からないことは自分で調べ、試行錯誤を繰り返す

---

## 今後の目標

子育て世代として、**教育系アプリやゲーム系アプリ**の開発に特に興味があります。

現在は個人開発で技術を磨いていますが、将来的には**チーム開発にも挑戦**し、
より大きなプロジェクトに携わっていきたいと考えています。

---

## スキル

### 開発技術
- React Native
- React
- TypeScript
- Firebase
- Expo

### 開発ツール
- Xcode
- Claude (AI開発アシスタント)
- Cursor (AIコードエディタ)
- Git / GitHub

### プラットフォーム
- iOS App Development
- App Store Connect

---

## 連絡先

お仕事のご依頼、ご相談などお気軽にご連絡ください。

- 📧 Email: kamui00002@yahoo.co.jp
- 📱 TEL: 080-4274-1778
- 💻 GitHub: [@kamui00002](https://github.com/kamui00002)

---

*最終更新: 2024年11月*